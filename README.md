...
Every notebook in `notebooks/` except `final_strategy.ipynb` has a matching
`.py` file, paired via [jupytext](https://jupytext.readthedocs.io/) percent
format — the `.py` file is a plain-text, diff-friendly mirror of the
notebook's code and markdown cells, not a separate script.
